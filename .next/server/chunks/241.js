...
const skills = (/* unused pure expression or super */ null && ([
    "Frontend Dev",
    "Backend Dev",
    "Solidity",
    "BizDev",
...
const hireus_services = (/* unused pure expression or super */ null && ([
...
;// CONCATENATED MODULE: ./middlewares/withToken.js
...
const withToken = (handler)=>{
    return async (req, res)=>{
        ...
        (0,external_jsonwebtoken_.verify)(token, process.env.JWT_SECRET, (err, signature)=>{
            if (err) return res.status(401).json("Invalid Token.");
            const msgHash = external_ethers_.utils.hashMessage(message_to_sign_join);
            const msgHashBytes = external_ethers_.utils.arrayify(msgHash);
            const account = external_ethers_.utils.recoverAddress(msgHashBytes, signature);
            if (account !== req.body["ETH Address"] && account !== req.body["eth_address"]) return res.status(401).json("Invalid Signature.");
